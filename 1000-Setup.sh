...
echo
echo "Configuring MAKEPKG to use all 6 cores"

sudo sed -i -e 's|[#]*MAKEFLAGS=.*|MAKEFLAGS="-j$(nproc)"|g' makepkg.conf
sudo sed -i -e 's|[#]*COMPRESSXZ=.*|COMPRESSXZ=(xz -c -T 6 -z -)|g' makepkg.conf
...
echo
echo "Enabling the SSHD service daemon so we can SSH"
...
echo
echo "Enabling the Avahi service daemon so we can bowse the Network"
...
echo
echo "Enabling the lightdm service daemon so we can log on graphically"
echo

echo "Edit /etc/lightdm/lightdm.conf [Seat:*] section."
echo "Uncomment greeter-session and change to:"
read -p "greeter-session=lightdm-slick-greeter"
